...
ggsave(filename = "D:/Peter/Analysis/KCNA2/BMK86-P1/KCNA1.svg", width = 3.5, height = 2)

cell_values$Time<-as.character(cell_values$Time)
cell_values$Time<-factor(cell_values$Time, levels = c("0","3","6","9","12"))
kcna1_tox<-lm(norm.Amp~Condition*Time, data = cell_values)
anova(kcna1_tox)
lsmeans(kcna1_tox, pairwise ~ Condition | Time, adjust = "tukey")
...
ggsave(filename = "D:/Peter/Analysis/KCNA2/BMK86-P1/KCNA1_inactivation.svg", width = 3.5, height = 2)

kcna1_tau<-lm(Tau~Condition*Time, data = cell_values)
anova(kcna1_tau)
lsmeans(kcna1_tau, pairwise ~ Condition | Time, adjust = "tukey")
...
data1<-readABF(cell10[1,1])
data1<-as.data.frame(data1,sweep=12)
data2<-readABF(cell10[6,1])
data2<-as.data.frame(data2,sweep=12)
data3<-readABF(cell10[11,1])
...
ggplot(data1,aes(`Time [s]`,`IN 0C [pA]`))+
  ...
  ylim(c(-4500/2,5000/2))+
  theme_prism(base_size = 14)+
  theme(axis.title.x=element_blank(),        axis.text.x=element_blank(),
        axis.line=element_blank(),
        axis.title.y=element_blank(),        axis.text.y=element_blank(),
        legend.position = "none")
ggsave(filename = "D:/Peter/Analysis/KCNA2/BMK86-P1/KCNA1_example.svg", width = 2, height = 2)
################################################################################################
##I/V curve over time 
################################################################################################
condition<-c(
...
ggsave(filename = "D:/Peter/Analysis/KCNA2/BMK86-P1/KCNA1_Toxin_act.png", width = 8, height = 6)
