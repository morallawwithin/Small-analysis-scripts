...
a<-ggplot(data,aes(x=time,y=V))+
  ...
  geom_segment(aes(x = max(time)-max(time)/8, y = -3.5e-04, xend = max(time), yend = -3.5e-04),lwd=2)+
  geom_segment(aes(x = max(time), y = -3.5e-04, xend = max(time), yend = -2.5e-04),lwd=2)
